...
function scssLinter() {
  return gulp.src( 'dev/static/styles/*.scss' )
    .pipe( stylelint( {
      fix: true,
      reporters: [
...
function pngSprite () {
  const spriteData = gulp.src( 'dev/static/img/sprites/png/*.png' )
    .pipe( spritesmith( {
      imgName: 'sprite.png',
...
function svgSpriteFunc() {
  return gulp.src( 'dev/static/img/sprites/svg/*.svg' )
    .pipe( svgmin( {
      js2svg: {
        pretty: true,
      }
...
const devTasks = gulp.parallel( fonts, pug2html, scss2css, javascript, jsVendors, imageMinify, svgSpriteFunc, pngSprite );
